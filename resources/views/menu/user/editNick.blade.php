...
<div class="container mx-auto p-3 mb-5 mx-auto bg-dark rounded-3 border border-primary shadow-lg">
    <form id="bio" action="{{route('putUserNick',$user->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="input-group mb-3">
            ...
            <textarea class="form-control form-control bg-dark text-white" form="bio" name="nick"
                placeholder="{{$user->nick}}" aria-label="{{$user->nick}}" aria-describedby="basic-addon1" required
                maxlength="450">{{$user->nick}}</textarea>
        </div>
        {{-- submit --}}
